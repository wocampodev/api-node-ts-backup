...
class MovementMySQLRepository implements MovementRepository {
    public async find(id: number): Promise<Movement | null> {
        const [data]: any[] = await MySQL().execute('SELECT * FROM wallet_movement WHERE id = ?', [id]);
    ...
    public async all(): Promise<Movement[]> {
        const [data]: any[] = await MySQL().execute('SELECT * FROM wallet_movement ORDER BY id DESC');

        return data as Movement[];
    }

    public async store(entry: Movement): Promise<void> {
        const { userId, type, amount } = entry;
        await MySQL().execute('INSERT INTO wallet_movement(user_id, type, amount, created_at) VALUES( ?, ?, ?, ? )', [
            userId,
            type,
            amount,
            new Date(),
    ...
    public async update(entry: Movement): Promise<void> {
        const { id, userId, type, amount } = entry;
        await MySQL().execute(
            'UPDATE wallet_movement SET user_id = ?, type = ?, amount = ?, updated_at = ? WHERE id = ?',
            [userId, type, amount, new Date(), id]
        );
    }

    public async remove(id: number): Promise<void> {
        await MySQL().execute('DELETE FROM wallet_movement WHERE id = ?', [id]);
    }
}
